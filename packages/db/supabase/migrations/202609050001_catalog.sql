...
do $$ begin
 if not exists(select 1 from pg_constraint where conname='current_release_belongs_to_game' and conrelid='public.games'::regclass) then
  alter table public.games add constraint current_release_belongs_to_game foreign key (id,current_release_id) references public.releases(game_id,id);
 end if;
end $$;
...
create or replace function public.promote_game(actor uuid, target_game uuid, target_release uuid, expected_generation integer)
