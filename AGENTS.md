...
call, command, spawned agent, cost figure, and failure shows in the run log as it happens. When you
...
apps/desktop/src/preload/    the only renderer↔main bridge (window.harnesses, window.loops)
...
Key files: `main/loop-runner.ts` (phase execution), `main/ledger.ts` (SQLite, both ledgers),
...
- New loop phase → the PHASE-001 checklist. Do not add a new control flow inside `loop-runner.ts`.
