...
function assetSeam(): string {
  return `Assets — the models already exist. The Asset Build phase sculpted them into \`src/assets/<name>.ts\`, one procedural factory per cast entry, before this round started, and the Reference Pack's \`cast.md\` says what each one is. Your job is to wire them up, never to sculpt or hand-edit them — if a model is wrong, say so in your report and it is regenerated. Only an entry the phase reported unbuildable is yours to model. Each factory returns a \`THREE.Group\` carrying \`userData.sculptRuntime\` (nodes, sockets, colliders, destructionGroups) and \`userData.rig\` (bones, skeleton, boneOrder, boneIndex, bound).
