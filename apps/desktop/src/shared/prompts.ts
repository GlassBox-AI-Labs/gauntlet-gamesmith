...
export const MOTION_FRAMES_RULE =
  'You read images; no tool plays video or audio for you. The frames under motion/, in filename order, are the readable form of the gameplay clip, which is kept as attributable provenance and as their source. Study them in sequence for movement and game feel, never claim to have watched or listened to the clip, and do not spend turns hunting for a tool that would let you.'
...
export function referenceReadingInstructions(referenceDir: string, mode: ReferenceMode = 'web'): string {
  ...
  return `Read ./${referenceDir}/README.md and ./${referenceDir}/research.md plus journey.md and story.md; VIEW the relevant stills and the extracted motion frames. ${MOTION_FRAMES_RULE} The frozen Reference Pack is required before implementation. ${suppliedReferenceInstructions(referenceDir)}`
}
...
Protocol:
${referenceMode === 'web' ? `1. Before planning, delegating, or writing code, read ./${referenceDir}/README.md, ./${referenceDir}/research.md, ./${referenceDir}/journey.md, and ./${referenceDir}/story.md; VIEW the relevant stills, motion frames, and ordered journey shots in the frozen Reference Pack. ${MOTION_FRAMES_RULE} Treat the Expert gameplay dossier in research.md as the authority for controls, mechanics, advanced techniques, enemies, fail/win states, difficulty, and progression — do not substitute memory. Do not replace or redownload the pack.` : `1. ${referenceReadingInstructions(referenceDir, referenceMode)}`}
