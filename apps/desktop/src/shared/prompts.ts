...
export function stripLeadContext(prompt: string): string {
  ...
  if (end < 0) throw new Error('Incomplete lead context envelope.')
  return preamble + body.slice(end + LEAD_CONTEXT_END.length)
}

export const QUEUED_LEAD_CHAT_PROMPT = 'Chat message queued for the continuing lead. The exact turn prompt is recorded when the lead is available.'

export function composeLeadChatPrompt(prompt: string, dispatch: LeadDispatch): string {
  return `Lead session: ${dispatch.reason}\n\n${prompt}`
}
...
export function buildSteeringPrompt(context: unknown): string {
  return `You are the continuing implementation lead for this game build, answering the operator in Chat between phase turns. This is the same lead conversation used for implementation. For THIS turn, only discuss the game and queue confirmed directions; do not implement, run commands, edit files, spawn workers, start a phase, or change the reference pack. You may inspect files with the read-only tools available. Prior implementation instructions apply only to implementation turns. Answer in first person about decisions you made, while distinguishing actual verification from saved reports or recovered memory. If the session was recovered, say when you lack firsthand context. Use the dated lead memory and conversation below to answer questions about plans, failed approaches, verification, and remaining work. State when a detail is missing or stale. Keep replies conversational; do not dump notebook fields, raw JSON, attempt IDs, or session IDs unless asked. Your whole response must be the JSON object described below. Clear directions enter the next implementation dispatch; explicit Resume includes pending directions when retrying implementation. Chat alone never starts work or restarts a stopped build.
