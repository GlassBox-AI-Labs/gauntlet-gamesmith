...
export function multiplayerInstructions(): string {
  return `Gamesmith multiplayer V1: when the requested game has online play, use ./platform/gamesmith.js (MultiplayerClient and PoseBuffer). Read ./platform/MULTIPLAYER.md. Keep single-player games independent of networking. Never create a Firebase/Supabase realtime client, private backend, or fake online success.
For an online game, ship gamesmith.multiplayer.json at the build root: {"version":1,"mode":"relay","maxPlayers":6,"sessionSeconds":180}. Create the client with MultiplayerClient.fromWindow(); the host injects window.gamesmith only into an authorized saved-round preview or published game. No host URLs, keys, publisher accounts, or login forms in game code. An ordinary Vite page has no bootstrap: show that online play needs the Gamesmith preview; do not claim bots prove multiplayer.
Use await client.join(displayName, optionalRoomId), then poll client.room() at most once per second. room.players contains {id,name,slot}; room.seed is shared; room.readyAt is the automatic lobby deadline. The first player can call client.room(true) to start sooner. room.startAt includes a countdown; room.endAt is the server-enforced finish. Use client.serverNow() for countdown, simulation start, and the 180-second match deadline. Start client.connect() when room.startAt exists. Keep local controls responsive; publish only compact motion/state records through client.publish(state), capped to 25 updates/second. client.session.playerId is the guest identity. This relay does not validate physics, combat, or scores: V1 is for low-stakes games without shared authoritative physics or rewards.
Subscribe with client.onState(snapshot), containing {playerId,seq,at,state}. Keep one PoseBuffer per remote player; push snapshot time/sequence, a pose {x,z,heading,s,vx,vz,speed}, and client.serverNow() as arrival. Render sample(client.serverNow()); interpolate the whole pose together, including ramp/track progress. Never apply each network packet directly to a rendered car or extrapolate forever. For other game genres, adapt their state rendering explicitly rather than assuming the pose helper handles all mechanics.
Show client.diagnostics status/errors and retry UI; client handles brief socket reconnections with the same guest ticket. Call client.leave() on leaving/results; dispose event subscriptions. No signup for guests. Use relative asset paths and tolerate unavailable browser storage inside the opaque-origin sandbox. Verify two real browser players in the same saved-round preview, movement under client.setCondition('internet'/'unstable') incoming-state delay, reconnect, and the server deadline. Report limits and failures visibly.`
}
