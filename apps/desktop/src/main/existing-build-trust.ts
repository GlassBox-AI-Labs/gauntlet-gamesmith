...
import { assertBuildWorkspaceIdentity } from './workspace-boundary'
import { snapshotBuildLedger } from './build-transfer'
...
export function captureExistingBuildTrust(
  db: DatabaseSync,
  build: BuildRecord,
  protectedRoots: readonly string[],
  validateHistory: (registry: DatabaseSync, workspace: string, portable: DatabaseSync) => void,
): string {
  assertBuildWorkspaceIdentity(build, protectedRoots)
  const current = db.prepare('SELECT * FROM builds WHERE id = ?').get(build.id)
  if (!current || current.workspace_dir !== build.workspaceDir || current.workspace_dev !== build.workspaceIdentity?.dev || current.workspace_ino !== build.workspaceIdentity?.ino) {
  ...
  const active = db.prepare(`SELECT builds.id FROM builds LEFT JOIN phase_attempts ON phase_attempts.build_id = builds.id
    ...
    WHERE builds.workspace_dir = ? AND (builds.status = 'running' OR phase_attempts.id IS NOT NULL) LIMIT 1`).get(build.workspaceDir)
  if (active) throw new Error('Active or unknown process ownership remains in this folder. Stop every build before trusting it.')
  const quarantined = db.prepare(`SELECT phase_attempts.id FROM phase_attempts JOIN builds ON builds.id = phase_attempts.build_id
    WHERE builds.workspace_dir = ? AND substr(phase_attempts.error, 1, ?) = ? LIMIT 1`).get(build.workspaceDir, UNKNOWN_OWNERSHIP.length, UNKNOWN_OWNERSHIP)
  if (quarantined) throw new Error('This workspace is quarantined after unknown process ownership.')
  const before = folderFingerprint(build.workspaceDir)
  const snapshot = snapshotBuildLedger(build.workspaceDir)
  try {
    ...
    try {
      ...
      validateHistory(db, build.workspaceDir, portable)
      for (const table of ['builds', 'phase_attempts', 'events'] as const) {
        ...
        const expected = db.prepare(`SELECT ${projection} FROM ${table} WHERE ${where} ORDER BY ${order}`).iterate(build.workspaceDir)
        ...
        for (;;) {
          ...
          if (left.done !== right.done || JSON.stringify(left.value) !== JSON.stringify(right.value)) {
            throw new Error('The portable history does not match the registered build history.')
          }
        }
        // Include every canonical column, even ones absent in an older portable schema.
        for (const row of db.prepare(`SELECT * FROM ${table} WHERE ${where} ORDER BY ${order}`).iterate(build.workspaceDir)) {
    ...
    assertBuildWorkspaceIdentity(build, protectedRoots)
    if (folderFingerprint(build.workspaceDir) !== before) throw new Error('The folder changed during trust validation. Try again.')
