...
describe('play safety', () => {
  ...
  it('refuses to open a URL that serves a blank page, and repairs the Vite config once', async () => {
    const dir = workspace()
    fs.writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ scripts: { dev: 'vite' } }))
    const child = new EventEmitter() as ChildProcess
    const stdout = new PassThrough()
    ...
    startPlay('loop-1', dir, null, null, notify, {
      ...
      groupIdentity: () => ['4242:original-start'],
      openExternal,
      bootProbe: async () => ({ ok: false, error: 'Game did not boot: HTTP 404 (http://127.0.0.1:4173/src/main.ts)' }),
    })
    stdout.write('ready at http://127.0.0.1:4173/')
    await new Promise((resolve) => setImmediate(resolve))
    await new Promise((resolve) => setImmediate(resolve))
    ...
    expect(playState('loop-1')).toEqual(expect.objectContaining({
      running: true,
      url: 'http://127.0.0.1:4173/',
      error: expect.stringContaining('did not boot'),
    }))
    // The generated config is rewritten, and the operator is told to restart.
    expect(playState('loop-1').error).toContain('Vite config was restored')
    expect(fs.readFileSync(path.join(dir, 'vite.config.js'), 'utf8')).toContain('Generated by Gauntlet')
  })
