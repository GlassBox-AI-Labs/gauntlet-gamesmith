...
export interface ClaudeLineResult {
  ...
  result?: {
    ...
    isError: boolean
    subtype: string | null
    /** Grok adds this; claude has no equivalent field. */
    errors?: string[]
...
export function translateClaudeLine(line: string): ClaudeLineResult | null {
  ...
  if (type === 'assistant') {
    ...
    for (const rawBlock of content) {
      ...
      if (block.type === 'text' && typeof block.text === 'string' && block.text.trim()) {
        ...
        // Grok emits this same wire format with its own tool names, so each
        // branch matches both vocabularies.
        if (name === 'Agent' || name === 'Task' || name === 'spawn_subagent') {
        ...
        } else if (name === 'Bash' || name === 'run_terminal_command') {
  ...
  if (type === 'result') {
    out.result = {
      ...
      errors: Array.isArray(obj.errors) ? (obj.errors as unknown[]).map(String) : undefined,
      raw: obj,
    }
