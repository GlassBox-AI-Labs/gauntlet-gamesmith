...
export function runWorkspaceFolderName(title: string): string {
  const slug = title
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export function createNewRunWorkspace(
  rootDir: string,
  title: string,
  ...
  const base = runWorkspaceFolderName(title)
  ...
  for (let attempt = 1; attempt <= MAX_FOLDER_ATTEMPTS; attempt += 1) {
    ...
    assertLoopWorkspaceIdentity(root, protectedRoots)
    return captureWorkspaceIdentity(candidate, protectedRoots)
  }
  throw new Error('Could not find an available project folder name for this run.')
}
