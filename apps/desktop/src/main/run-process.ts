...
import { runsDir } from './harness-env'
...
export interface RunProcessMeta {
  version: 1
  pid: number
...
export function processMetaPath(workspaceDir: string, runId: string): string {
  if (!isRecordId(runId)) throw new Error('Invalid run id for process metadata.')
  return path.join(path.resolve(workspaceDir), WORKSPACE_METADATA_DIR, 'runs', `${runId}.json`)
}

function processStartingMetaPath(workspaceDir: string, runId: string): string {
  if (!isRecordId(runId)) throw new Error('Invalid run id for starting process metadata.')
  return path.join(path.resolve(workspaceDir), WORKSPACE_METADATA_DIR, 'runs', `${runId}.starting.json`)
}

export function processStreamPaths(workspaceDir: string, runId: string): { outPath: string; errPath: string } {
  if (!isRecordId(runId)) throw new Error('Invalid run id for process streams.')
  // Path derivation must be side-effect free. Callers validate the registered
  // workspace identity before using these names; launch creates the owned run
  // directory explicitly in prepareProcessMeta.
  const dir = path.join(path.resolve(workspaceDir), WORKSPACE_METADATA_DIR, 'runs')
  return { outPath: path.join(dir, `${runId}.out.ndjson`), errPath: path.join(dir, `${runId}.err.log`) }
}
...
function assertLaunchBoundary(workspaceDir: string, marker: StartingProcessMeta): void {
  ...
  const runRoot = fs.lstatSync(path.dirname(marker.outPath))
  if (
    runRoot.isSymbolicLink()
    || !runRoot.isDirectory()
    || runRoot.dev !== marker.runsDev
    || runRoot.ino !== marker.runsIno
  ) throw new Error('Run-stream directory changed identity during process launch.')
...
export function prepareProcessMeta(
  ...
  const runRootPath = runsDir(workspaceDir, true)
  const runRoot = fs.lstatSync(runRootPath)
  if (runRoot.isSymbolicLink() || !runRoot.isDirectory()) throw new Error('Run-stream directory is not a real directory.')
  const paths = processStreamPaths(workspaceDir, runId)
  ...
  exclusiveWrite(processStartingMetaPath(workspaceDir, runId), marker, assertBoundary)
  return marker
}
...
function readProcessSnapshot(pid: number): ProcessSnapshot | null {
  ...
  // a CLI launcher hands off to its real binary and would reject a valid run.
...
export function completeProcessMeta(
  ...
  const streams = capturedStreams ?? (() => {
    ...
    if (!out.isFile() || out.isSymbolicLink() || out.nlink !== 1 || !err.isFile() || err.isSymbolicLink() || err.nlink !== 1) {
      throw new Error('Run streams are not singly linked regular files.')
    }
  ...
  exclusiveWrite(processMetaPath(workspaceDir, runId), meta, () => assertLaunchBoundary(workspaceDir, marker))
  return meta
}
...
export function readProcessMeta(workspaceDir: string, runId: string): ProcessMetaReadResult {
  try {
    runsDir(workspaceDir, false)
  } catch {
    return { meta: null, error: 'process metadata is missing' }
  }
  const completed = processMetaPath(workspaceDir, runId)
  const starting = processStartingMetaPath(workspaceDir, runId)
  let file = completed
  try {
  ...
  const expected = processStreamPaths(workspaceDir, runId)
  ...
  try {
    ...
    const runRoot = fs.realpathSync(runsDir(workspaceDir, false))
    ...
    if (!isContained(workspace, runRoot) || !isContained(runRoot, out) || !isContained(runRoot, err)) {
      return { meta: null, error: 'process stream paths escape the workspace run directory' }
    }
    if (
  ...
  return { meta: raw as unknown as RunProcessMeta, error: null }
}

export function processMatches(meta: RunProcessMeta): boolean {
  return safePid(meta.pid) && readProcessIdentity(meta.pid) === meta.processIdentity
}

export function processGroupAlive(meta: RunProcessMeta): boolean {
...
export interface ProcessInterruptDeps {
  ...
  identityMatches(meta: RunProcessMeta): boolean
...
export function interruptProcessGroup(
  ...
  if (!deps.identityMatches(meta)) {
    report(`SIGINT skipped: process group ${meta.pid} is gone or its leader identity no longer belongs to this run.`)
