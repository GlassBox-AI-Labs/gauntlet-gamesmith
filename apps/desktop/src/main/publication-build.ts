...
export async function buildPublication(directory: string, record: (job: BuildJob) => void, log: (text: string) => void): Promise<string> {
  ...
  record(job)
  const stat = fs.statSync(directory), marker = prepareProcessMeta(directory, runId, Date.now(), { dev: stat.dev, ino: stat.ino })
  ...
  try {
    ...
    meta = completeProcessMeta(directory, runId, marker, child.pid)
  } catch (error) {
  ...
  let timedOut = false
  const interrupt = (): Promise<void> => new Promise((resolve, reject) => interruptCapturedProcessGroup(meta.pid, meta.groupIdentities, log, outcome => outcome === 'gone' ? resolve() : reject(new Error('Build process ownership is unresolved. Retry after it exits.'))))
  const timer = setTimeout(() => { timedOut = true; void interrupt().catch(e => log(e.message)) }, 120000)
  const watcher = setInterval(() => { try { scan() } catch (e) { log(`Build stream inspection failed: ${e instanceof Error ? e.message : 'unknown error'}`) } }, 250)
  timer.unref(); watcher.unref()
  try {
    record({ ...job, status: 'running' })
    log(`Publishing build: npm ${args.join(' ')}. Full build streams are retained in the saved-round checkout.`)
...
export async function recoverPublicationBuild(job: BuildJob, log: (text: string) => void): Promise<void> {
  ...
  const result = readProcessMeta(job.directory, job.runId)
