...
import { MAX_CONTEXT_BYTES, MAX_CONTEXT_FILE_BYTES, MAX_CONTEXT_FILES, type RunAttachment } from '../shared/attachments'
...
interface StoredAttachment { item: RunAttachment; files: StoredFile[]; source: string; dev: number; ino: number }
...
export function createRunAttachments(protectedRoots: () => string[]) {
  ...
  return {
    add(value: unknown): RunAttachment[] {
      ...
      for (const input of value) {
        ...
        const item: RunAttachment = { id: crypto.randomUUID(), name, kind: stat.isDirectory() ? 'folder' : IMAGE_MIMES[path.extname(name).toLowerCase()] ? 'image' : 'file', bytes: files.reduce((sum, file) => sum + file.bytes.length, 0), files: files.length, skipped }
