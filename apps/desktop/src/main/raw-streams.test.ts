...
describe('parseReadStreamInput', () => {
  it('accepts typed primary and agent requests', () => {
    expect(parseReadStreamInput({ attemptId, stream: 'stdout', offset: 0 })).toEqual({ attemptId, stream: 'stdout', offset: 0 })
    expect(parseReadStreamInput({ attemptId, stream: 'agent', agentId: 'child:physics', offset: 42, identity: '1:2' })).toEqual({
      attemptId,
      stream: 'agent',
  ...
  it('rejects traversal, coercion, invalid cursors, and agents without separate streams', () => {
    expect(() => parseReadStreamInput({ attemptId: '../../build', stream: 'stdout', offset: 0 })).toThrow('Invalid build id')
    expect(() => parseReadStreamInput({ attemptId, stream: 'stdout', agentId: 'child:physics', offset: 0 })).toThrow('must not include')
    expect(() => parseReadStreamInput({ attemptId, stream: 'agent', agentId: 'orchestrator', offset: 0 })).toThrow('separate raw stream')
    expect(() => parseReadStreamInput({ attemptId, stream: 1, offset: 0 })).toThrow('Invalid raw stream kind')
    expect(() => parseReadStreamInput({ attemptId, stream: 'stdout', offset: -1 })).toThrow('non-negative integer')
    expect(() => parseReadStreamInput({ attemptId, stream: 'stdout', offset: 1.5 })).toThrow('non-negative integer')
    expect(() => parseReadStreamInput({ attemptId, stream: 'stdout', offset: 0, identity: '../file' })).toThrow('identity is invalid')
  })
})
...
describe('resolveRawStreamPath', () => {
  it('resolves build, archived child, workflow, and Codex streams from ids', () => {
    ...
    for (const dir of [attemptRoot, childRoot, workflowRoot, codexRoot]) fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(attemptRoot, `${attemptId}.out.ndjson`), '{}\n')
    ...
    expect(resolveRawStreamPath(input, { attemptId, stream: 'stdout' })).toBe(fs.realpathSync(path.join(attemptRoot, `${attemptId}.out.ndjson`)))
    expect(resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'child:physics' })).toBe(fs.realpathSync(path.join(childRoot, 'physics.codex.jsonl')))
    expect(resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'wf:wf_build:a1' })).toBe(fs.realpathSync(path.join(workflowRoot, 'agent-a1.jsonl')))
    expect(resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'codex:thread-abc' })).toBe(fs.realpathSync(path.join(codexRoot, 'rollout-date-thread-abc.jsonl')))
  })
  ...
  it('refuses a symlink that escapes the owned root', () => {
    ...
    fs.symlinkSync(outside, path.join(attemptRoot, `${attemptId}.out.ndjson`))
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'stdout' })).toThrow('not a regular file')
  })
  ...
  it('refuses hard-linked raw files', () => {
    ...
    const outside = path.join(root!, 'outside.jsonl')
    fs.writeFileSync(outside, 'secret')
    fs.linkSync(outside, path.join(attemptRoot, `${attemptId}.out.ndjson`))
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'stdout' })).toThrow('unsafe hard link')
  })

  it('refuses an owned stream directory redirected outside its workspace', () => {
    const input = roots()
    const outside = path.join(root!, 'outside-builds')
  ...
  it('revalidates a registry workspace before revealing after a protected-root swap', () => {
    ...
    expect(() => resolveProtectedRawStreamPath(input, { attemptId, stream: 'stdout' }, [protectedRoot])).toThrow(
  ...
  it('refuses an owned stream directory redirected elsewhere within its owner', () => {
    ...
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'wf:wf_build:a1' })).toThrow(
      'unsafe path component',
    )
  })

  it('does not fall back to a newer live child stream for a historical build', () => {
    const input = roots()
    input.allowLiveChildStream = false
    const childRoot = path.join(input.workspaceDir, '.gauntlet-gamesmith', 'agents')
    fs.mkdirSync(childRoot, { recursive: true })
    fs.writeFileSync(path.join(childRoot, 'physics.codex.jsonl'), '{}\n')
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'child:physics' })).toThrow('not found')
  })
  ...
  it('rejects ambiguous child harnesses and Codex rollout suffixes', () => {
    ...
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'child:physics' })).toThrow('ambiguous')
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'codex:thread' })).toThrow('ambiguous')
  })
  ...
  it('bounds Codex session traversal depth before recursive stack growth', () => {
    ...
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'codex:thread' })).toThrow('depth limit')
  })
  ...
  it('rejects a symlinked Codex sessions root before traversing it', () => {
    ...
    expect(() => resolveRawStreamPath(input, { attemptId, stream: 'agent', agentId: 'codex:thread' })).toThrow(
      'unsafe path component',
    )
  })
})
