...
const roots: string[] = []
function root(): string { const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'run-context-')); roots.push(dir); return dir }
afterEach(() => { for (const dir of roots.splice(0)) fs.rmSync(dir, { recursive: true, force: true }) })
describe('run attachment snapshots', () => {
  it('publishes original bytes and provenance after the source changes or disappears', () => {
    ...
    const result = prepared.publish(workspace, 'reference/loop-1')
    expect(fs.readFileSync(path.join(workspace, result.paths[0]), 'utf8')).toBe('Original context')
    const manifest = JSON.parse(fs.readFileSync(path.join(workspace, 'reference/loop-1/supplied/manifest.json'), 'utf8'))
    expect(manifest.files[0]).toMatchObject({ original: 'notes.txt', bytes: 16 })
    expect(JSON.stringify(manifest)).not.toContain(source)
    expect(referencePackFingerprint(workspace, 'reference/loop-1/supplied')).toBe(result.fingerprint)
    fs.writeFileSync(path.join(workspace, result.paths[0]), 'tampered')
    expect(referencePackFingerprint(workspace, 'reference/loop-1/supplied')).not.toBe(result.fingerprint)
  })
  it('never follows symlinks or copies credential/generated files during folder ingestion', () => {
    ...
    const store = createRunAttachments(() => []); const [item] = store.add([source])
  ...
  it('rejects unknown IDs, private roots, oversized files, and partial batches', () => {
    ...
    expect(() => createRunAttachments(() => [source]).add([file])).toThrow('private')
    const store = createRunAttachments(() => [])
