...
describe('run process metadata', () => {
  ...
  it('persists a starting marker, then validates exact contained streams and PID identity', () => {
    ...
    const marker = prepareProcessMeta(dir, runId, Date.now(), workspaceIdentity(dir))
    fs.writeFileSync(marker.outPath, '')
    fs.writeFileSync(marker.errPath, '')
    expect(readProcessMeta(dir, runId).error).toContain('did not finish')

    const meta = completeProcessMeta(dir, runId, marker, process.pid, (pid) => testSnapshot(pid, marker.startedAtMs))
    expect(readProcessMeta(dir, runId)).toEqual({ meta, error: null })
  ...
  it('rejects a reused PID or a process that is not the detached group leader', () => {
    ...
    const runId = randomUUID()
    const marker = prepareProcessMeta(dir, runId, Date.now(), workspaceIdentity(dir))
    expect(() => completeProcessMeta(dir, runId, marker, 42, () => ({
  ...
  it('rejects forged paths and malformed offsets before recovery can read them', () => {
    ...
    fs.writeFileSync(marker.errPath, '')
    const meta = completeProcessMeta(dir, runId, marker, process.pid, (pid) => testSnapshot(pid, marker.startedAtMs))
    fs.writeFileSync(processMetaPath(dir, runId), JSON.stringify({ ...meta, outPath: '/etc/passwd' }))
    expect(readProcessMeta(dir, runId).error).toContain('exact-path')
    fs.writeFileSync(processMetaPath(dir, runId), JSON.stringify({ ...meta, childOffsets: { '../escape.codex.jsonl': 1 } }))
    expect(readProcessMeta(dir, runId).error).toContain('schema')
    fs.writeFileSync(processMetaPath(dir, runId), JSON.stringify({ ...meta, workflowOffsets: { '../session/agent-a.jsonl': 1 } }))
    expect(readProcessMeta(dir, runId).error).toContain('schema')
  })
  ...
  it('rejects a symlink planted at an otherwise exact stream path', () => {
    ...
    fs.writeFileSync(target, 'not this run')
    fs.writeFileSync(marker.outPath, '')
    fs.writeFileSync(marker.errPath, '')
    const meta = completeProcessMeta(dir, runId, marker, process.pid, (pid) => testSnapshot(pid, marker.startedAtMs))
    ...
    expect(readProcessMeta(dir, runId)).toEqual({ meta: null, error: 'process streams must not be symbolic links' })
  })

  it('rejects hard-linked process metadata before parsing it', () => {
    const dir = workspace()
    const runId = randomUUID()
    const file = processMetaPath(dir, runId)
    fs.mkdirSync(path.dirname(file), { recursive: true })
    const outside = path.join(dir, 'outside-meta.json')
    fs.writeFileSync(outside, '{}')
    fs.linkSync(outside, file)

    expect(readProcessMeta(dir, runId).error).toContain('unlinked regular file')
  })
  ...
  it('never overwrites a completed-metadata target planted at exclusive publication', () => {
    ...
    fs.writeFileSync(marker.outPath, '')
    fs.writeFileSync(marker.errPath, '')
    const target = processMetaPath(dir, runId)
    ...
    expect(() => completeProcessMeta(dir, runId, marker, process.pid, (pid) => testSnapshot(pid, marker.startedAtMs))).toThrow(/EEXIST/)
  ...
  it('does not publish completed metadata through a replaced workspace root', () => {
    ...
    expect(() => completeProcessMeta(dir, runId, marker, process.pid, (pid) => testSnapshot(pid, marker.startedAtMs))).toThrow(
      /Workspace root changed identity/,
    )
    expect(fs.existsSync(processMetaPath(dir, runId))).toBe(false)
  })
