...
/**
 * Vanilla Vite. TypeScript already maps `.js` specifiers onto `.ts` files;
 * a custom resolve plugin that rewrites those specifiers also rewrites
 * `node_modules/.vite/deps/*.js` into missing `*.ts` URLs, and Play comes up
 * blank.
 *
 * A new workspace gets this file. Later Gauntlet Loop updates replace it only
 * when the on-disk file is still an unedited generated copy. A config the
 * game has grown is left alone. Play restores this file only if that grown
 * config blanks the Play URL.
 */
export const VITE_CONFIG = `import { defineConfig } from 'vite'

// Generated by Gauntlet Loop.
...
/** Unedited copies from earlier Gauntlet Loop versions, so we can upgrade them in place. */
export const PREVIOUS_GENERATED_VITE_CONFIGS = [
...
// Generated by Gauntlet Loop. Play restores this file when the game is launched.
export default defineConfig({
  server: { host: '127.0.0.1' },
  preview: { host: '127.0.0.1' },
  build: { target: 'es2022' },
...
export type ViteConfigSync = 'created' | 'refreshed' | 'kept'

/** Write or upgrade the generated Vite config; do not touch a file the game has grown. */
export function syncViteConfig(workspaceDir: string): ViteConfigSync {
  const full = path.join(workspaceDir, 'vite.config.js')
  fs.mkdirSync(workspaceDir, { recursive: true })
  if (!fs.existsSync(full)) {
    fs.writeFileSync(full, VITE_CONFIG)
    return 'created'
  }
  const before = fs.readFileSync(full, 'utf8')
  if (before === VITE_CONFIG) return 'kept'
  if (PREVIOUS_GENERATED_VITE_CONFIGS.includes(before)) {
    fs.writeFileSync(full, VITE_CONFIG)
    return 'refreshed'
  }
  return 'kept'
...
/** Last resort for Play: the current config blanked the page, so put the default back. */
export function rewriteViteConfig(workspaceDir: string): void {
  const full = path.join(workspaceDir, 'vite.config.js')
  fs.mkdirSync(workspaceDir, { recursive: true })
  fs.writeFileSync(full, VITE_CONFIG)
}
 ...
 * The Vite config is the same unless it is still an unedited generated copy,
 * which is upgraded in place so a later Gauntlet Loop default can ship. The app's
 * files — the contract and the gate — are rewritten every round, because a
 * contract that drifts is not a contract and a gate a worker can edit to pass
 * is not a gate.
 */
export function scaffoldEngine(workspaceDir: string, expectedWorkspace?: OwnedWorkspaceIdentity): ScaffoldResult {
  ...
  const vite = syncViteConfig(workspaceDir)
  if (vite === 'created') created.push('vite.config.js')
  else if (vite === 'refreshed') refreshed.push('vite.config.js')
