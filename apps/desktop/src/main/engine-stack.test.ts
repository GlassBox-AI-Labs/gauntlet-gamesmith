...
import { PREVIOUS_GENERATED_VITE_CONFIGS, rewriteViteConfig, scaffoldEngine, syncViteConfig, VITE_CONFIG } from './engine-stack'
...
describe('engineContract', () => {
  ...
  it('forbids a Vite resolve plugin and hiding a blank Play page behind preview', () => {
    const contract = engineContract()
    expect(contract).toContain('vite.config.js')
    expect(contract).toContain('You may extend `vite.config.js`')
    expect(contract).toContain('unedited generated config may be updated in place')
    expect(contract).toContain('Do not add a Vite resolve plugin')
    expect(contract).toContain('npm run dev')
    expect(contract).toContain('vite preview')
    expect(contract).not.toContain('Do not edit `vite.config.js`')
  })
...
describe('scaffoldEngine', () => {
  ...
  it('never clobbers a Vite config the game has grown, the same as the manifest', () => {
    const dir = workspace()
    scaffoldEngine(dir)
    const vitePath = path.join(dir, 'vite.config.js')
    const grown = `import { defineConfig } from 'vite'\nexport default defineConfig({ plugins: [{ name: 'wasm' }] })\n`
    fs.writeFileSync(vitePath, grown)

    const second = scaffoldEngine(dir)

    expect(second.refreshed).not.toContain('vite.config.js')
    expect(fs.readFileSync(vitePath, 'utf8')).toBe(grown)
  })

  it('upgrades an unedited generated Vite config to the current default', () => {
    const dir = workspace()
    scaffoldEngine(dir)
    const vitePath = path.join(dir, 'vite.config.js')
    const previous = PREVIOUS_GENERATED_VITE_CONFIGS[0]
    expect(previous).toBeTruthy()
    fs.writeFileSync(vitePath, previous!)

    const second = scaffoldEngine(dir)

    expect(second.refreshed).toContain('vite.config.js')
    expect(fs.readFileSync(vitePath, 'utf8')).toBe(VITE_CONFIG)
  })
})

describe('syncViteConfig', () => {
  it('keeps the current generated file and does not report a refresh', () => {
    const dir = workspace()
    expect(syncViteConfig(dir)).toBe('created')
    expect(syncViteConfig(dir)).toBe('kept')
    expect(fs.readFileSync(path.join(dir, 'vite.config.js'), 'utf8')).toBe(VITE_CONFIG)
  })
})

describe('rewriteViteConfig', () => {
  it('replaces a poisoned resolve plugin when Play cannot boot', () => {
    const dir = workspace()
    scaffoldEngine(dir)
    const vitePath = path.join(dir, 'vite.config.js')
    fs.writeFileSync(
...
export default defineConfig({
  plugins: [{ name: 'js-specifier-to-ts', resolveId() {} }],
})
`,
    )

    rewriteViteConfig(dir)
    const restored = fs.readFileSync(vitePath, 'utf8')

    expect(restored).toContain('Generated by Gauntlet Loop')
    expect(restored).not.toContain('js-specifier-to-ts')
    expect(restored).not.toContain('resolveId')
    expect(restored).not.toContain('extensionAlias')
  })
})
