...
describe('scaffoldEngine', () => {
  it('creates the manifest, the layout and the gate', () => {
    ...
    expect(fs.readFileSync(path.join(dir, 'platform/gamesmith.js'), 'utf8')).toContain('TransformBuffer')
    expect(fs.readFileSync(path.join(dir, 'platform/gamesmith.js'), 'utf8')).toContain('SnapshotBuffer')
    expect(fs.readFileSync(path.join(dir, 'platform/MULTIPLAYER.md'), 'utf8')).toContain('180 seconds of active shared play')
