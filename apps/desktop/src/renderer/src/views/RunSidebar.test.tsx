...
describe('RunSidebar', () => {
  ...
  it('reserves room for run and round status labels', () => {
    const snapshot = {
      loop: {
        id: 'loop-1',
        title: 'A deliberately long run title that must yield space to status',
        workspaceDir: '/tmp/example',
        status: 'running',
      },
      runs: [{ id: 'run-1', round: 1, status: 'running', verdict: null }],
    }
    const markup = renderToStaticMarkup(
      <RunSidebar
