...
export function BuildSidebar({
  ...
  hasMoreHistories,
  hasNewerHistories,
}: BuildSidebarProps): React.JSX.Element {
  const checkedCount = checkedBuilds.size
  ...
  return (
    <aside className="flex h-screen w-[252px] min-w-0 shrink-0 flex-col overflow-hidden border-r border-[#2a2626] bg-[#141112]">
      <div className="px-3 pb-3 pt-5">
        <button type="button" onClick={onNewBuild} disabled={busy} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-[14px] font-medium text-[#ded9d6] transition-colors hover:bg-white/[0.05] hover:text-white disabled:cursor-not-allowed disabled:opacity-40">
          <Plus className="size-4" /> Build
        </button>
        <button type="button" onClick={onImportBuild} disabled={busy} className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-[13px] text-[#918a87] transition-colors hover:bg-white/[0.05] hover:text-[#ded9d6] disabled:cursor-not-allowed disabled:opacity-40">
          <Download className="size-4" /> Import build
        </button>
      </div>
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto px-2.5 py-4">
        {editing ? (
          <div className="mb-2 flex items-center gap-1">
            <span className="pl-2 pr-1"><BuildCheckbox checked={allChecked} mixed={checkedCount > 0 && !allChecked} label={allChecked ? 'Select none' : 'Select all builds'} onToggle={onToggleAllChecked} /></span>
          ...
          <div className="mb-2 flex items-center justify-between pl-2">
            <span className="text-[13px] font-medium text-[#8d8784]">Builds</span>
            <button type="button" onClick={onToggleEditing} disabled={snapshots.length === 0} aria-label="Edit builds" title="Select builds to delete or turn into a report" className="grid size-6 place-items-center rounded-md text-[#68615f] hover:bg-white/[0.05] hover:text-[#ded9d6] disabled:cursor-not-allowed disabled:text-[#463f3e]"><Pencil className="size-3.5" /></button>
          </div>
        )}
        ...
        <div className="grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-1">
            ...
            return (
              <div key={buildId}>
                <div className={`group flex w-full min-w-0 max-w-full items-center overflow-hidden rounded-lg pr-2 transition-colors ${selected ? 'bg-[#302b2b] text-[#eeeae7]' : 'text-[#aaa4a1] hover:bg-white/[0.035] hover:text-[#ded9d6]'}`}>
                  {editing && <span className="pl-2 pr-1"><BuildCheckbox checked={checkedBuilds.has(buildId)} label={`Select ${label}`} onToggle={() => onToggleChecked(buildId)} /></span>}
                  <button type="button" onClick={() => onToggleBuild(buildId)} className="grid size-8 shrink-0 place-items-center rounded-md text-[#716b68] hover:text-[#c9c3c0]" aria-expanded={open} aria-controls={`sidebar-rounds-${buildId}`} aria-label={`${open ? 'Collapse' : 'Expand'} ${label}`}>
          ...
          {snapshots.length === 0 && <p className="px-2 py-3 text-xs leading-relaxed text-[#68615f]">Your builds will appear here.</p>}
        </div>
        ...
        <div className="grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-1">
          ...
          {reports.length === 0 && <p className="px-2 py-3 text-xs leading-relaxed text-[#68615f]">Compare builds side by side: select builds with the pencil, then choose Report.</p>}
        </div>
      </div>
