...
The **Artifacts** tab on a run lists its generated folders (project folder, `src/assets` factories, `.img2threejs` sculptor evidence, the frozen Reference Pack, and `critique/` captures) with item counts and an **Open** button for the OS file browser, and shows a visual gallery of every procedural asset factory next to its rendered sculptor preview. The gallery reads rendered evidence images through the loopback media server only; it never executes project code.
