...
(`@gauntlet/desktop`). Root scripts delegate to that package. Electron hosts the build executor in
...
flowchart LR
  ...
  CLI -->|source and evidence| Workspace[Build workspace]
  CLI -->|streams| Runner
...
`window.harnesses`, `window.builds`, and `window.onboarding`. Main treats received values as
...
A **build** is the whole job. In [`shared/build.ts`](../apps/desktop/src/shared/build.ts), a
`BuildRecord` owns its goal, workspace, configuration, budget, and overall status. A build runs in
numbered **rounds**; a **phase** is one role within one round; a `PhaseAttempt` is one execution of
one phase, with its own status, prompt, session, metrics, and result.
...
[`main/build-runner.ts`](../apps/desktop/src/main/build-runner.ts) owns start, dispatch, supervision,
...
[`main/attempt-transition.ts`](../apps/desktop/src/main/attempt-transition.ts) commits terminal attempt
...
1. **Create and reference input.** Main creates a fresh project folder and publishes selected
   attachments under `reference/<build-id>/supplied/`. `referenceMode: 'web'` queues a Reference
...
2. **Reference Study, round 0.** The reference protocol validates the mode-specific pack using
   ...
   `reference/<build-id>/` is fingerprinted before implementation. Later phases verify it remains
...
4. **Continuation or stop.** If budget and round limits permit, a critique attempt is queued against
   that revision. Completing implementation at `round >= maxRounds` exhausts the build immediately,
...
notes in fresh sessions; notes never prove verification or override current requirements (ADR-038).
...
The sidebar identifies the conversation as **Chat with orchestrator**, and replies as **Orchestrator** (ADR-040).
Complete attempt and recovery events remain in the existing activity log (ADR-039, refining ADR-036/037/038).

### Asset compatibility

New builds do not queue a separate `assets` attempt. `wantedCast` in the runner selects the cast
...
`PhaseRole` still includes `'assets'`, and the legacy execution/parser/plan path remains to recover
old attempts. Do not infer a phase in a new build from that union alone. The
...
The build decides **what role runs next**; a harness plan decides **how to invoke the selected CLI**.
...
[`main/attempt-process.ts`](../apps/desktop/src/main/attempt-process.ts) and runner supervision bind launches
...
`verdict-<attempt-id>.json`, enforcing a bounded regular file, freshness, schema, and the expected source
...
[`main/build-transfer.ts`](../apps/desktop/src/main/build-transfer.ts) handles export/import and related
...
tailers project CLI output into timestamped `BuildLogLine` events. The ledger stores those events
...
`build-runner-lifecycle.test.ts`, `phase-contracts.test.ts`, `parse-verdict.test.ts`, and `ledger.test.ts`
